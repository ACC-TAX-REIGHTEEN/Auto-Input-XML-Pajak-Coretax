...
def bersihkan_data_faktur_final(input_file, output_file):
    print("--> Memulai pembersihan data kedua")
    ...
    if 'TGL' in df_bersih.columns:
        df_bersih['TGL'] = pd.to_datetime(df_bersih['TGL'], dayfirst=True, format='%d/%m/%y', errors='coerce')
        df_bersih['TGL'] = df_bersih['TGL'].dt.strftime('%d/%m/%Y')
